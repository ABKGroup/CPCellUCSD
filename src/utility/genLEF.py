...
def main(argv, argc):
    ...
    for line in lines:
        ...
        if read_mode and line.startswith("b{6"):
            ...
            floats = [float(x) for x in floats]
            tmp_pt = Point(floats[1:])
            # print("gp -- lx: " + str(tmp_pt.get_lx()) + " ly: " + str(tmp_pt.get_ly()) + " ux: " + str(tmp_pt.get_ux()) + " uy: " + str(tmp_pt.get_uy()))
            tmp_pin = Pin(
                llx=tmp_pt.get_lx(),
                lly=tmp_pt.get_ly(),
                urx=tmp_pt.get_ux(),
        ...
        if read_mode and line.startswith("t{6"):
            ...
            for pin in tmp_pin_list:
                if (
                    ...
                    print("setting gp pin name", pin_name, float(floats[0]), float(floats[1]))
